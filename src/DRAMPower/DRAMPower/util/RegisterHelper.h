...
decltype(auto) dispatchBank(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups, const Mapping& map, Func&& func, Ctx&&... ctx) {
    ...
    return std::invoke(std::forward<Func>(func), std::forward<Ctx>(ctx)...,
        ...
        cmd.timestamp - offset);
}

template<typename GroupSel, typename Cmd, typename Group, typename Mapping, typename Func, typename... Ctx>
decltype(auto) dispatchGroup(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups, const Mapping& map, Func&& func, Ctx&&... ctx) {
    assert(cmd.timestamp >= offset);
    const std::size_t groupIdx = checked(map.group(cmd), groups.size(), "Invalid group coordinate");
    auto& group = groups[groupIdx];
    return std::invoke(std::forward<Func>(func), std::forward<Ctx>(ctx)...,
...
decltype(auto) bankHandler(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups,
                           const Mapping& map, Func&& func, Ctx&&... ctx)
{   // (Group&, Bank&, timestamp)
    return detail::dispatchBank<detail::ByRef, detail::ByRef>(
        cmd, offset, groups, map, std::forward<Func>(func), std::forward<Ctx>(ctx)...);
}

template<typename Cmd, typename Group, typename Mapping, typename Func, typename... Ctx>
decltype(auto) bankHandlerIdx(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups,
                              const Mapping& map, Func&& func, Ctx&&... ctx)
...
template<typename Cmd, typename Group, typename Mapping, typename Func, typename... Ctx>
decltype(auto) bankGroupHandler(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups,
                                const Mapping& map, Func&& func, Ctx&&... ctx)
...
template<typename Cmd, typename Group, typename Mapping, typename Func, typename... Ctx>
decltype(auto) groupHandler(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups,
                            const Mapping& map, Func&& func, Ctx&&... ctx)
{
    // (Group&, timestamp)
...
template<typename Cmd, typename Group, typename Mapping, typename Func, typename... Ctx>
decltype(auto) groupHandlerIdx(const Cmd& cmd, timestamp_t offset, std::vector<Group>& groups,
                               const Mapping& map, Func&& func, Ctx&&... ctx)
{
    // (groupIdx, timestamp)
