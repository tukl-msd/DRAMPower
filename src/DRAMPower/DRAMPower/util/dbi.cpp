...
void DBI::dispatchResetCallback(timestamp_t timestamp, bool read) const {
    ...
    assert(!m_lastBurst.init || m_lastBurst.consumed || m_lastInversionSize >= getChunksPerWidth()); // Ensure there is at least one chunk in the last inversion
    if (!m_lastBurst.init || m_lastBurst.consumed) {
        // Call before last burst -> nothing to reset
        // or Last burst is already consumed
        return;
...
std::optional<const uint8_t *> DBI::updateDBI(timestamp_t timestamp, std::size_t n_bits, const uint8_t* data, bool read)
{
    ...
    m_lastBurst.consumed = false; // Mark the last burst as not consumed
