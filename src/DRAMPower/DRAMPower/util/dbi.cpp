...
const std::vector<DBI::State_t>& DBI::getInversionState() const
{
    return m_lastInvert;
}
...
void DBI::dispatchResetCallback(timestamp_t timestamp, bool read) {
    ...
    assert(!m_lastBurst || timestamp > m_lastBurst->end); // Ensure timestamp is after the last burst end
    assert(!m_lastBurst || m_lastInversionSize >= getChunksPerWidth()); // Ensure there is at least one chunk in the last inversion
    if (!m_lastBurst // No previous burst
        || (m_lastBurst->end + 1 == timestamp && m_lastBurst->read == read)) // Seamless burst
    {
        return; 
    }
    ...
    if (nullptr != m_changeCallback) {
        ...
        timestamp_t t = m_lastBurst->end + 1;
        ...
        for (std::size_t chunk = start; chunk < m_lastInversionSize; ++chunk) {
            ...
            if (m_lastInvert[chunk]) {
                ...
                m_lastInvert[chunk] = false; // Reset to idle state
            }
        }
    }
}
...
std::optional<const uint8_t *> DBI::updateDBI(timestamp_t timestamp, std::size_t n_bits, const uint8_t* data, bool read)
{
    ...
    m_lastBurst = LastBurst_t{m_lastBurstEnd, read};
