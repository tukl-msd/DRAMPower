...
interface_energy_info_t InterfaceCalculation_DDR4::calcClockEnergy(const SimulationStats &stats) {
    ...
    result.controller.staticPower =
        calc_static_power(2 * stats.clockStats.ones, impedances_.R_eq_ck, 0.5 * t_CK_, VDD_, 1);
    result.controller.dynamicPower =
        calc_dynamic_power(2 * stats.clockStats.zeroes_to_ones, impedances_.C_total_ck, VDD_);
...
interface_energy_info_t InterfaceCalculation_DDR4::calcDQSEnergy(const SimulationStats &stats) {
    ...
    result.dram.dynamicPower +=
        calc_dynamic_power(preposreadzero_to_one * (readcount - preposreadseamless), impedances_.C_total_dqs, VDD_);
    result.controller.dynamicPower +=
        calc_dynamic_power(preposwritezero_to_one * (writecount - preposwriteseamless), impedances_.C_total_dqs, VDD_);
    ...
    result.controller.dynamicPower +=
        calc_dynamic_power(2 * NumDQsPairs *stats.writeDQSStats.zeroes_to_ones, impedances_.C_total_dqs, VDD_);
