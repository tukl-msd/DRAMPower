...
public:
    ...
    , m_implicitCommandInserter(std::move(implicitCommandInserter))
    {
        ...
        if (memSpec.bitWidth > 4) {
            ...
            m_dataBus.withExtensionRead<util::bus_extensions::BusExtensionDBI>(callback);
            m_dataBus.withExtensionWrite<util::bus_extensions::BusExtensionDBI>(callback);
            m_dbiread.resize(n_dbi_pins, util::Pin{util::PinState::H});
            m_dbiwrite.resize(n_dbi_pins, util::Pin{util::PinState::H});
        }
        registerPatterns();
    }
