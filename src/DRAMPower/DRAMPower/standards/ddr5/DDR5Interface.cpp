...
void DDR5Interface::registerPatterns() {
    ...
    // PRESB
    m_patternHandler.get().registerPattern<CmdType::PRESB>({
        H, H, L, H, L, CID3, BA0, BA1, V, V, H, CID0, CID1, CID2
    });
    // REFSB
    m_patternHandler.get().registerPattern<CmdType::REFSB>({
    ...
    // NOP
    m_patternHandler.get().registerPattern<CmdType::NOP>({
        H, H, H, H, H, V, V, V, V, V, V, V, V, V
    });
}
...
void DDR5Interface::handleOverrides(size_t length, bool read)
{
    ...
    switch(length) {
        case 8:
            m_patternHandler.get().getEncoder().settings.removeSetting(pattern_descriptor::C10);
            if(read)
            {
                // Read
