...
void DDR5Core::handlePreSameBank(Rank & rank, std::size_t bank_id, timestamp_t timestamp) {
    auto bank_id_inside_bg = bank_id % m_memSpec.get().banksPerGroup;
    for(unsigned bank_group = 0; bank_group < m_memSpec.get().numberOfBankGroups; bank_group++) {
        auto & bank = rank.banks[bank_group * m_memSpec.get().banksPerGroup + bank_id_inside_bg];
...
void DDR5Core::handleRefSameBank(Rank & rank, std::size_t bank_id, timestamp_t timestamp) {
    ...
    for(unsigned bank_group = 0; bank_group < m_memSpec.get().numberOfBankGroups; bank_group++) {
        auto & bank = rank.banks[bank_group * m_memSpec.get().banksPerGroup + bank_id_inside_bg];
        handleRefreshOnBank(rank, bank, timestamp, m_memSpec.get().memTimingSpec.tRFCsb, bank.counter.refSameBank);
    }
}
