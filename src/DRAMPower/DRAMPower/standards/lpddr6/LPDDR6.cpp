...
namespace DRAMPower {
    ...
    void LPDDR6::registerCommands() {
        ...
        // ACT
        this->registerBankHandler<CmdType::ACT>(&LPDDR6::handleAct);
        ...
        if (memSpec.bank_arch == MemSpecLPDDR6::MBG) {
            act_pattern[9] = BG0;
            act_pattern[10] = BG1;
        } else if (memSpec.bank_arch == MemSpecLPDDR6::M8B) {
            act_pattern[10] = V;
        }
        this->registerPattern<CmdType::ACT>(act_pattern);
        this->registerInterfaceMember<CmdType::ACT>(&LPDDR6::handleInterfaceCommandBus);
        // PRE
        this->registerBankHandler<CmdType::PRE>(&LPDDR6::handlePre);
        ...
        this->registerInterfaceMember<CmdType::PRE>(&LPDDR6::handleInterfaceCommandBus);
        // PREA
        this->registerRankHandler<CmdType::PREA>(&LPDDR6::handlePreAll);
        ...
        this->registerInterfaceMember<CmdType::PREA>(&LPDDR6::handleInterfaceCommandBus);
        // REFB
        this->registerBankHandler<CmdType::REFB>(&LPDDR6::handleRefPerBank);
        ...
        this->registerInterfaceMember<CmdType::REFB>(&LPDDR6::handleInterfaceCommandBus);
        // RD
        this->registerBankHandler<CmdType::RD>(&LPDDR6::handleRead);
        ...
        // RDA
        this->registerBankHandler<CmdType::RDA>(&LPDDR6::handleReadAuto);
        ...
        // WR
        this->registerBankHandler<CmdType::WR>(&LPDDR6::handleWrite);
        ...
        // WRA
        this->registerBankHandler<CmdType::WRA>(&LPDDR6::handleWriteAuto);
        ...
        // REFP2B
        this->registerBankGroupHandler<CmdType::REFP2B>(&LPDDR6::handleRefPerTwoBanks);
        if (memSpec.bank_arch == MemSpecLPDDR6::MBG || memSpec.bank_arch == MemSpecLPDDR6::M16B) {
            this->registerPattern<CmdType::REFP2B>(refb_pattern);
            this->registerInterfaceMember<CmdType::REFP2B>(&LPDDR6::handleInterfaceCommandBus);
        }
        // REFA
        this->registerRankHandler<CmdType::REFA>(&LPDDR6::handleRefAll);
        ...
        this->registerInterfaceMember<CmdType::REFA>(&LPDDR6::handleInterfaceCommandBus);
        // SREFEN
        this->registerRankHandler<CmdType::SREFEN>(&LPDDR6::handleSelfRefreshEntry);
        ...
        this->registerInterfaceMember<CmdType::SREFEN>(&LPDDR6::handleInterfaceCommandBus);
        // SREFEX
        this->registerRankHandler<CmdType::SREFEX>(&LPDDR6::handleSelfRefreshExit);
        ...
        this->registerInterfaceMember<CmdType::SREFEX>(&LPDDR6::handleInterfaceCommandBus);
        // PDEA
        this->registerRankHandler<CmdType::PDEA>(&LPDDR6::handlePowerDownActEntry);
        ...
        this->registerInterfaceMember<CmdType::PDEA>(&LPDDR6::handleInterfaceCommandBus);
        // PDEP
        this->registerRankHandler<CmdType::PDEP>(&LPDDR6::handlePowerDownPreEntry);
        ...
        this->registerInterfaceMember<CmdType::PDEP>(&LPDDR6::handleInterfaceCommandBus);
        // PDXA
        this->registerRankHandler<CmdType::PDXA>(&LPDDR6::handlePowerDownActExit);
        ...
        this->registerInterfaceMember<CmdType::PDXA>(&LPDDR6::handleInterfaceCommandBus);
        // PDXP
        this->registerRankHandler<CmdType::PDXP>(&LPDDR6::handlePowerDownPreExit);
        ...
        this->registerInterfaceMember<CmdType::PDXP>(&LPDDR6::handleInterfaceCommandBus);
        // DSMEN
        this->registerRankHandler<CmdType::DSMEN>(&LPDDR6::handleDSMEntry);
        ...
        this->registerInterfaceMember<CmdType::DSMEN>(&LPDDR6::handleInterfaceCommandBus);
        // DSMEX
        this->registerRankHandler<CmdType::DSMEX>(&LPDDR6::handleDSMExit);
        ...
        this->registerInterfaceMember<CmdType::DSMEX>(&LPDDR6::handleInterfaceCommandBus);
        // EOS
        routeCommand<CmdType::END_OF_SIMULATION>([this](const Command &cmd) { this->endOfSimulation(cmd.timestamp); });
    }

// Getters for CLI
    uint64_t LPDDR6::getBankCount() {
        return memSpec.numberOfBanks;
    }

    uint64_t LPDDR6::getRankCount() {
        return memSpec.numberOfRanks;
    }

    uint64_t LPDDR6::getDeviceCount() {
    return memSpec.numberOfDevices;
}

// Update toggling rate
    void LPDDR6::enableTogglingHandle(timestamp_t timestamp, timestamp_t enable_timestamp) {
    ...
    void LPDDR6::enableBus(timestamp_t timestamp, timestamp_t enable_timestamp) {
...
// Interface
    void LPDDR6::handleInterfaceOverrides(size_t length, bool /*read*/)
    {
...
// Core
    void LPDDR6::handleAct(Rank &rank, Bank &bank, timestamp_t timestamp) {
        bank.counter.act++;
    ...
    void LPDDR6::handlePre(Rank &rank, Bank &bank, timestamp_t timestamp) {
    ...
    void LPDDR6::handlePreAll(Rank &rank, timestamp_t timestamp) {
        for (auto &bank: rank.banks) {
            handlePre(rank, bank, timestamp);
        }
    }

    void LPDDR6::handleRefPerBank(Rank & rank, Bank & bank, timestamp_t timestamp) {
        handleRefreshOnBank(rank, bank, timestamp, memSpec.memTimingSpec.tRFCPB, bank.counter.refPerBank);
    }

    void LPDDR6::handleRefPerTwoBanks(Rank & rank, std::size_t bank_id, timestamp_t timestamp) {
    ...
    void LPDDR6::handleRefAll(Rank &rank, timestamp_t timestamp) {
        for (auto& bank : rank.banks) {
    ...
    void LPDDR6::handleRefreshOnBank(Rank & rank, Bank & bank, timestamp_t timestamp, uint64_t timing, uint64_t & counter){
        ++counter;
        if (!rank.isActive(timestamp)) {
            rank.cycles.act.start_interval(timestamp);
        }
    ...
    void LPDDR6::handleRead(Rank&, Bank &bank, timestamp_t) {
        ++bank.counter.reads;
    }

    void LPDDR6::handleReadAuto(Rank &rank, Bank &bank, timestamp_t timestamp) {
    ...
    void LPDDR6::handleWrite(Rank&, Bank &bank, timestamp_t) {
        ++bank.counter.writes;
    }

    void LPDDR6::handleWriteAuto(Rank &rank, Bank &bank, timestamp_t timestamp) {
    ...
    void LPDDR6::handleSelfRefreshEntry(Rank &rank, timestamp_t timestamp) {
    ...
    void LPDDR6::handleSelfRefreshExit(Rank &rank, timestamp_t timestamp) {
        assert(rank.memState == MemState::SREF);
        rank.cycles.sref.close_interval(timestamp);
        rank.memState = MemState::NOT_IN_PD;
    }

    void LPDDR6::handlePowerDownActEntry(Rank &rank, timestamp_t timestamp) {
    ...
    void LPDDR6::handlePowerDownActExit(Rank &rank, timestamp_t timestamp) {
    ...
    void LPDDR6::handlePowerDownPreEntry(Rank &rank, timestamp_t timestamp) {
        auto earliestPossibleEntry = this->earliestPossiblePowerDownEntryTime(rank);
        auto entryTime = std::max(timestamp, earliestPossibleEntry);
        addImplicitCommand(entryTime, [this, &rank, entryTime]() {
    ...
    void LPDDR6::handlePowerDownPreExit(Rank &rank, timestamp_t timestamp) {
        // TODO: Is this computation necessary?
        auto earliestPossibleExit = this->earliestPossiblePowerDownEntryTime(rank);
        auto exitTime = std::max(timestamp, earliestPossibleExit);
    ...
    void LPDDR6::handleDSMEntry(Rank &rank, timestamp_t timestamp) {
        assert(rank.memState == MemState::SREF);
    ...
    void LPDDR6::handleDSMExit(Rank &rank, timestamp_t timestamp) {
...
// Stats
    ...
    SimulationStats LPDDR6::getStats() {
        return getWindowStats(getLastCommandTime());
    }

// Core helpers
    timestamp_t LPDDR6::earliestPossiblePowerDownEntryTime(Rank &rank) {
