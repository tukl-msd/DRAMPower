...
uint64_t LPDDR6Encoder::encode(const LPDDR6TargetCoordinate& targetCoordinate, const std::vector<pattern_descriptor_LPDDR6::t>& pattern, const BasePatternEncoderOverrides<pattern_descriptor_LPDDR6::t>&, const uint64_t, const LPDDR6PatternExtraData& extraData) {
    using namespace pattern_descriptor_LPDDR6;
        ...
        std::bitset<2> bank_group_bits(targetCoordinate.bankGroup & 0x3);
        assert((0 == targetCoordinate.dbank % extraData.numberOfBankGroups) && "Invalid bank configuration");
        auto dbankGroup = targetCoordinate.dbank / extraData.numberOfBankGroups;
        std::bitset<2> dbank_group_bits(dbankGroup & 0x3);
        ...
        for (const auto descriptor : pattern) {
            ...
            case BG1:
                ...
                break;
            case DBG0:
                bitset[n] = dbank_group_bits[0];
                break;
            case DBG1:
                bitset[n] = dbank_group_bits[1];
                break;
            // Column bits
            case C0:
                assert((24 == extraData.currentBurstLength || 48 == extraData.currentBurstLength) && "Invalid burstlength");
                bitset[n] = (24 == extraData.currentBurstLength) ? column_bits[0] : false;
                break;
            case C1:
