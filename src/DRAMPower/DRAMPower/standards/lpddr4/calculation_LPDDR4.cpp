	...
	energy_t Calculation_LPDDR4::calcEnergy(timestamp_t timestamp, LPDDR4 & dram) { 
        ...
        for(auto vd  : {MemSpecLPDDR4::VoltageDomain::VDD}) {
            ...
            for (std::size_t b = 0; b < dram.memSpec.numberOfBanks; ++b) {
                ...
                energy.bank_energy[b].E_bg_act += E_BG_act_star(B, VDD, approx_IDD3N, I_rho,
                                                               stats.bank[b].cycles.activeTime() * t_CK);
                energy.bank_energy[b].E_bg_pre += E_BG_pre(B, VDD, IDD2N, stats.total.cycles.pre * t_CK);
                energy.bank_energy[b].E_RD += E_RD(VDD, IDD4R, IDD3N, BL, DR, t_CK, bank.counter.reads);
                energy.bank_energy[b].E_WR += E_WR(VDD, IDD4W, IDD3N, BL, DR, t_CK, bank.counter.writes);
                energy.bank_energy[b].E_RDA += E_RD(VDD, IDD4R, IDD3N, BL, DR, t_CK, bank.counter.readAuto);
                energy.bank_energy[b].E_WRA += E_WR(VDD, IDD4W, IDD3N, BL, DR, t_CK, bank.counter.writeAuto);
            ...
            energy.E_sref += VDD * IDD6 * stats.total.cycles.selfRefresh * t_CK;
            energy.E_PDNA += VDD * IDD3P * stats.total.cycles.powerDownAct * t_CK;
            energy.E_PDNP += VDD * IDD2P * stats.total.cycles.powerDownPre * t_CK;

            energy.E_bg_act_shared += E_BG_act_shared(VDD, I_rho, stats.total.cycles.act * t_CK);
        }
