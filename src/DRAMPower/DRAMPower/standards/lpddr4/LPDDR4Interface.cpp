...
LPDDR4Interface::LPDDR4Interface(const MemSpecLPDDR4 &memSpec, implicitCommandInserter_t&& implicitCommandInserter, patternHandler_t &patternHandler)
...
void LPDDR4Interface::registerPatterns() {
    ...
    // REFB
    m_patternHandler.get().registerPattern<CmdType::REFB>({
        L, L, L, H, L, L,
        BA0, BA1, BA2, V, V, V,
    });
    // RD
...
void LPDDR4Interface::handleOverrides(size_t length, bool /*read*/)
{
    ...
    switch(length) {
        ...
        case 16:
            m_patternHandler.get().getEncoder().settings.removeSetting(pattern_descriptor::C4);
            m_patternHandler.get().getEncoder().settings.updateSettings({
                {pattern_descriptor::C3, PatternEncoderBitSpec::L},
                {pattern_descriptor::C2, PatternEncoderBitSpec::L},
