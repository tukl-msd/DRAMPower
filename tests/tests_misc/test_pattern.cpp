...
TEST_F(PatternTest, Test_Override_Low)
{
	...
	auto cmd = Command{0, CmdType::ACT, { 1,2,3,4,17} };
	auto result = encoder.encode(cmd.targetCoordinate, pattern, 0);
...
TEST_F(PatternTest, Test_Override_Last)
{
	...
	// Bank, Bank Group, Rank, Row, Column
	auto cmd = Command{0, CmdType::ACT, { 1,2,3,4,17} };
	auto result = encoder.encode(cmd.targetCoordinate, pattern, init_pattern);
...
TEST_F(PatternTest, Test_Override_2_Patterns)
{
	...
	cmd = Command{0, CmdType::ACT, { 7,3,3,7,17} };
	result = encoder.encode(cmd.targetCoordinate, pattern, result);
