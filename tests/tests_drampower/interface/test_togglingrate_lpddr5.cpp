   ...
   public:
    LPDDR5_TogglingRate_Tests() {
        test_patterns.push_back({
            ...
            {3,  CmdType::WR,  {0, 1, 0, 0, 0, 8}, nullptr, datasize_bits},
            {12, CmdType::RD,  {0, 1, 0, 0, 0, 8}, nullptr, datasize_bits},
...
TEST_F(LPDDR5_TogglingRate_Tests, Pattern_1) {
    ...
    ddr->doCoreInterfaceCommand({5, CmdType::WR,  {0, 1, 0, 0, 0, 8}, wr_data, SZ_BITS(wr_data)});
    ddr->doCoreInterfaceCommand({14, CmdType::RD, {0, 1, 0, 0, 0, 8}, rd_data, SZ_BITS(rd_data)});
    ...
    ddr->doCoreInterfaceCommand({35, CmdType::WR,  {0, 1, 0, 0, 0, 8}, nullptr, 16*8}); // burst length = 16
    ddr->doCoreInterfaceCommand({44, CmdType::RD,  {0, 1, 0, 0, 0, 8}, nullptr, 16*8}); // burst length = 16
