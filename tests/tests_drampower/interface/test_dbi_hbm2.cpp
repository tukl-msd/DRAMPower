...
using DRAMPower::HBM2Command;
using DRAMPower::HBM2TargetCoordinate;
   ...
   public:
    HBM2_DBI_Tests() {
        test_patterns.push_back({
            // bank, bankgroup, rank, row, column, stack, pseudoChannel
            // bank, rank, row, column, stack, pseudoChannel
            {0,  CmdType::ACT, {1, 2, 0,  0, 0}},
            {4,  CmdType::WR,  {1, 0, 16, 0, 0}, wr_data, SZ_BITS(wr_data)},
            {11, CmdType::RD,  {1, 0, 16, 0, 0}, rd_data, SZ_BITS(rd_data)},
            // Non seamless read
            {14, CmdType::RD,  {1, 0, 16, 0, 0}, rd_data, SZ_BITS(rd_data)},
            {20, CmdType::PRE, {1, 2, 0,  0, 0}},
        ...
        test_patterns.push_back({
            ...
            {11, CmdType::RD,  {1, 0, 16, 0, 0}, rd_data, SZ_BITS(rd_data)},
            // Seamless read
            {13, CmdType::RD,  {1, 0, 16, 0, 0}, rd_data, SZ_BITS(rd_data)}, // Seamless read
            {20, CmdType::PRE, {1, 2, 0, 0, 0}},
            {24, CmdType::END_OF_SIMULATION},
        });
    ...
    void runCommands(const std::vector<HBM2Command> &commands) {
        for (const HBM2Command &command : commands) {
            ddr->getCore().doCommand(command);
            ddr->getInterface().doCommand(command);
        }
    }

    std::vector<std::vector<HBM2Command>> test_patterns;
