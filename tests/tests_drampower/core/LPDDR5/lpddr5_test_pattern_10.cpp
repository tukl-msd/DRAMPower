...
protected:
    ...
    std::vector<Command> testPattern = {
        // Timestamp,   Cmd,  { Channel, Bank, BG, Rank }
            {   10, CmdType::ACT,    { 0, 0, 0, 0 }},
            {   30, CmdType::PRE,    { 0, 0, 0, 0 }},
            {   45, CmdType::SREFEN, { 0, 0, 0, 0 }},
            {   70, CmdType::DSMEN,  { 0, 0, 0, 0 }},
            {   85, CmdType::DSMEX,  { 0, 0, 0, 0 }},
            {   95, CmdType::SREFEX, { 0, 0, 0, 0 }},
...
TEST_F(DramPowerTest_LPDDR5_10, Counters_and_Cycles){
    ...
    for(uint64_t b = 0; b < ddr->memSpec.numberOfBanks; b++)  ASSERT_EQ(stats.bank[b].cycles.selfRefresh, 10);

    // Check bank specific DSM cycle count
    for(uint64_t b = 0; b < ddr->memSpec.numberOfBanks; b++)  ASSERT_EQ(stats.bank[b].cycles.deepSleepMode, 15);
}
