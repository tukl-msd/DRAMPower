...
TEST_F(DramPowerTest_LPDDR5_4, Counters_and_Cycles){
    ...
    // Check cycles count
    ASSERT_EQ(stats.total.cycles.act, 50);
...
TEST_F(DramPowerTest_LPDDR5_4, Energy) {
    ...
    ASSERT_EQ(std::round(total_energy.E_bg_act), 1633);
    ASSERT_EQ(std::round(energy.E_bg_act_shared), 1587);
    ASSERT_EQ(std::round(total_energy.E_bg_pre), 623);
    ASSERT_EQ(std::round(total_energy.total()), 3742);
}
