/* Note
This test is based on lpddr6_test_pattern_4.cpp and assumes the correctness of the test.
...
class DramPowerTest_LPDDR6_MultiDevice : public ::testing::Test {
protected:
    // Test pattern
...
TEST_F(DramPowerTest_LPDDR6_MultiDevice, Counters_and_Cycles){
...
TEST_F(DramPowerTest_LPDDR6_MultiDevice, Energy) {
    for (const auto& command : testPattern) {
        ddr->doCoreCommand(command);
    }
    ...
    ASSERT_EQ(std::round(total_energy.E_RD*1e12), 3375);
    ...
    ASSERT_EQ(std::round(total_energy.total()*1e12), 18692);
}
