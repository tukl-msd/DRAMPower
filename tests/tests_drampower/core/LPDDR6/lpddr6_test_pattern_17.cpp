...
class DramPowerTest_LPDDR6_17 : public ::testing::Test {
protected:
    // Test pattern
	std::vector<Command> testPattern = {
		{  0, CmdType::REFDB, { 0, 0, 0 }},
		{  5, CmdType::REFDB, { 1, 0, 0 }},
		{ 40, CmdType::REFDB, { 4, 0, 0 }},
		{ 45, CmdType::ACT,    { 0, 0, 0 }},
		{ 70, CmdType::PRE,    { 0, 0, 0 }},
		{ 80, CmdType::REFDB, { 0, 0, 0 }},
    ...
    virtual void SetUp()
    {
		...
		memSpec->memPowerSpec[0].iDD5PDBX = 10009e-3;
...
TEST_F(DramPowerTest_LPDDR6_17, Test)
{
	// TODO validate
	...
	ASSERT_EQ(stats.bank[4].counter.refDualBanks, 1);
	ASSERT_EQ(stats.bank[9].counter.refDualBanks, 1);
	ASSERT_EQ(stats.bank[12].counter.refDualBanks, 1);
...
TEST_F(DramPowerTest_LPDDR6_17, CalcEnergy)
{
	...
	ASSERT_EQ(std::round(total_energy.E_bg_act*1e12), 3093);
	ASSERT_EQ(std::round(energy.E_bg_act_shared*1e12), 2493);
	ASSERT_EQ(std::round(energy.bank_energy[0].E_bg_act*1e12), 200);
	ASSERT_EQ(std::round(energy.bank_energy[1].E_bg_act*1e12), 67);
	ASSERT_EQ(std::round(energy.bank_energy[2].E_bg_act*1e12), 0);
	ASSERT_EQ(std::round(energy.bank_energy[3].E_bg_act*1e12), 0);
	ASSERT_EQ(std::round(energy.bank_energy[4].E_bg_act*1e12), 67);
	...
	ASSERT_EQ(std::round(total_energy.E_ref_DB*1e12), 2334);
	ASSERT_EQ(std::round(energy.bank_energy[0].E_ref_DB*1e12), 583);
	ASSERT_EQ(std::round(energy.bank_energy[1].E_ref_DB*1e12), 292);
	ASSERT_EQ(std::round(energy.bank_energy[2].E_ref_DB*1e12), 0);
	ASSERT_EQ(std::round(energy.bank_energy[3].E_ref_DB*1e12), 0);
	ASSERT_EQ(std::round(energy.bank_energy[4].E_ref_DB*1e12), 292);
