...
class DramPowerTest_LPDDR6_11 : public ::testing::Test {
protected:
    // Test pattern
    std::vector<Command> testPattern = {
            ...
            {   40, CmdType::REFDB,  { 9, 0, 0 }},
            {   50, CmdType::REFDB,  { 5, 0, 0 }},
...
TEST_F(DramPowerTest_LPDDR6_11, Counters_and_Cycles){
...
TEST_F(DramPowerTest_LPDDR6_11, Energy) {
    for (const auto& command : testPattern) {
        ddr->doCoreCommand(command);
    }
    ...
    ASSERT_EQ(std::round(total_energy.E_ref_DB*1e12), 1215);
    ...
    ASSERT_EQ(std::round((total_energy.total() + energy.E_sref + energy.E_PDNA + energy.E_PDNP)*1e12), 5663);
}
