...
class DramPowerTest_LPDDR6_3 : public ::testing::Test {
protected:
    // Test pattern
...
TEST_F(DramPowerTest_LPDDR6_3, Counters_and_Cycles){
...
TEST_F(DramPowerTest_LPDDR6_3, Energy) {
    for (const auto& command : testPattern) {
        ddr->doCoreCommand(command);
    }
    ...
    ASSERT_EQ(std::round(total_energy.total()*1e12), 2651);
}
