...
class DramPowerTest_LPDDR6_9 : public ::testing::Test {
protected:
    // Test pattern
...
TEST_F(DramPowerTest_LPDDR6_9, Counters_and_Cycles){
...
TEST_F(DramPowerTest_LPDDR6_9, Energy) {
    for (const auto& command : testPattern) {
        ddr->doCoreCommand(command);
    }
    ...
    ASSERT_EQ(std::round(total_energy.E_RD*1e12), 675);
    ...
    ASSERT_EQ(std::round((total_energy.total() + energy.E_sref + energy.E_PDNA + energy.E_PDNP)*1e12), 4231);
}
