...
class DramPowerTest_LPDDR6_5 : public ::testing::Test {
protected:
    // Test pattern
...
TEST_F(DramPowerTest_LPDDR6_5, Counters_and_Cycles){
...
TEST_F(DramPowerTest_LPDDR6_5, Energy) {
    for (const auto& command : testPattern) {
        ddr->doCoreCommand(command);
    }
    ...
    ASSERT_EQ(std::round(total_energy.total()*1e12), 6617);
}
