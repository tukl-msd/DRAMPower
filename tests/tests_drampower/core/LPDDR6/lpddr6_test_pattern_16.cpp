...
TEST_F(DramPowerTest_LPDDR6_16, Test)
{
	...
	ASSERT_EQ(stats.bank[2].counter.refPerTwoBanks, 1);
	ASSERT_EQ(stats.bank[3].counter.refPerTwoBanks, 0);

	ASSERT_EQ(stats.bank[8].counter.refPerTwoBanks, 2);
	ASSERT_EQ(stats.bank[9].counter.refPerTwoBanks, 1);
	ASSERT_EQ(stats.bank[10].counter.refPerTwoBanks, 1);
	ASSERT_EQ(stats.bank[11].counter.refPerTwoBanks, 0);
...
TEST_F(DramPowerTest_LPDDR6_16, CalcEnergy)
{
    ...
    ASSERT_EQ(std::round(total_energy.E_ref_2B*1e12), 600);
