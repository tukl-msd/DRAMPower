...
class DramPowerTest_LPDDR6_6 : public ::testing::Test {
protected:
    // Test pattern
...
TEST_F(DramPowerTest_LPDDR6_6, Counters_and_Cycles){
...
TEST_F(DramPowerTest_LPDDR6_6, Energy) {
    for (const auto& command : testPattern) {
        ddr->doCoreCommand(command);
    }
    ...
    ASSERT_EQ(std::round(total_energy.E_RD*1e12), 225);
    ASSERT_EQ(std::round(total_energy.E_RDA*1e12), 225);
    ...
    ASSERT_EQ(std::round(total_energy.total()*1e12), 6375);
}
