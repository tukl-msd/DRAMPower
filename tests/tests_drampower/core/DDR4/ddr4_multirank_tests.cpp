  ...
  public:
    ...
    void runCommands(const std::vector<Command> &commands) {
        for (const Command &command : commands) {
            ddr->doCoreInterfaceCommand(command);
        }
    }
...
TEST_F(DDR4_MultirankTests, Pattern_2) {
    ...
    runCommands({
        ...
        { 15,  CmdType::RDA,  {0, 0, 0, 0}, rd_data, SZ_BITS(rd_data)},
        { 20,  CmdType::ACT,  {0, 3, 0, 1}},  // r1
        { 35,  CmdType::RD,   {0, 3, 0, 1}, rd_data, SZ_BITS(rd_data)},  // r1
        { 40,  CmdType::RD,   {0, 0, 0, 0}, rd_data, SZ_BITS(rd_data)},
